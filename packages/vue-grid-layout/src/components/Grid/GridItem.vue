...
import {EventsData, Breakpoints, Layout} from "../../types"
...
function tryMakeDraggable() {
  ...
  if (draggable.value && !props.static) {
    ...
    if (!dragEventSet.value) {
      ...
      interactObj.value.on("dragstart dragmove dragend", function (event: any) {
        handleDrag(event)
      })
    }
  } else {
...
function tryMakeResizable() {
  ...
  if (resizable.value && !props.static) {
    ...
    if (!resizeEventSet.value) {
      ...
      interactObj.value.on("resizestart resizemove resizeend", function (event: any) {
        handleResize(event)
      })
    }
  } else {
